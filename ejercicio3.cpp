...
int main()
{
    ...
    // Set the position of the light and the marker to the same position
    osg::Vec3 lightCords(-5.0f, -5.0f, -5.0f);

    osg::Vec4f lightPosition(osg::Vec4f(lightCords, 1.0f));
    osg::ref_ptr<osg::Light> myLight = new osg::Light;
    myLight->setLightNum(0);
    myLight->setPosition(lightPosition);

    // Set light color to red for visibility
    myLight->setAmbient(osg::Vec4(1.0f, 0.0f, 0.0f, 1.0f)); // Ambient light
    myLight->setDiffuse(osg::Vec4(1.0f, 0.0f, 0.0f, 1.0f)); // Diffuse light
    ...
    //Light markers: small cube
    osg::ref_ptr<osg::Geode> lightMarkerGeode = new osg::Geode;
    lightMarkerGeode->addDrawable(new osg::ShapeDrawable(new osg::Box(osg::Vec3(lightCords), 0.8f)));

    // Add light and cube marker to group
    group->addChild(lightGroup);
    group->addChild(lightMarkerGeode);
